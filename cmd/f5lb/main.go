...
import (
	...
	"github.com/caicloud/loadbalancer-provider/providers/f5lb"
)
...
func Run(opts *Options) error {
	...
	log.Infof("Provider Running with %+v", opts)
	...
	if err != nil {
		log.Errorf("Create clientset error: %v", err)
		return err
	}
	...
	if err != nil {
		log.Fatalf("Can not find loadbalancer resource %s/%s", opts.LoadBalancerNamespace, opts.LoadBalancerName)
		return err
	}
	if lb.Spec.Providers.F5 == nil {
		log.Fatalf("Not a F5 Proivder %s/%s", opts.LoadBalancerNamespace, opts.LoadBalancerName)
		return err
	}

	elb, err := f5lb.New(clientset, lb)
	if err != nil {
		return err
	}
